...
function renderIngresos() {
  ...
  // SECURITY: escapeHTML() prevents XSS from user-controlled person names
    ...
    <div style="display:flex;flex-direction:column;gap:.75rem">
      ${[{n:p1,v:totP1},{n:p2,v:totP2},{n:'Total',v:tot}].map(x=>`
        <div style="display:flex;justify-content:space-between;align-items:center;padding:.6rem .8rem;background:rgba(255,255,255,0.04);border-radius:.6rem">
          <span style="font-size:.85rem;color:var(--white)">${escapeHTML(x.n)}</span>
      ...
      ${tot>0 ? `
        <div style="font-size:.75rem;color:var(--gray);text-align:center;margin-top:.25rem">
          ${escapeHTML(p1)}: ${(totP1/tot*100).toFixed(1)}% · ${escapeHTML(p2)}: ${(totP2/tot*100).toFixed(1)}%
        </div>` : ''}
    </div>`;
  ...
  tbody.innerHTML = rows.length ? rows.map(i=>`
    <tr>
      <td>${MONTHS_ES[parseInt(i.month.slice(5))-1]} ${escapeHTML(i.month.slice(0,4))}</td>
      <td style="font-weight:500">${escapeHTML(i.person)}</td>
      <td><span class="chip" style="background:rgba(45,106,106,0.2);color:var(--teal-light)">${escapeHTML(i.type)}</span></td>
